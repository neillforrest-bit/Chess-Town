    ...
    <div style={{ position: 'relative', width: '100%', height: '100%', backgroundColor: '#050008', color: 'white', fontFamily: 'Georgia, Times New Roman, serif', overflow: 'hidden', boxSizing: 'border-box' }}>
      ...
      {scene === 'CHESTER_REVEAL' && (
        <div className="chester-intro-page chester-reveal" style={{ width: '100%', height: '100%', display: 'flex', flexDirection: isLandscape ? 'row' : 'column', alignItems: 'center', justifyContent: 'center', backgroundColor: '#020005', padding: isLandscape ? '1rem 4rem' : '2rem', position: 'relative', overflow: 'hidden', gap: isLandscape ? '4rem' : '0' }}>
          ...
          <div className="chester-sigil" style={{ position: 'relative', width: isLandscape ? '42vh' : 'min(38vw, 240px)', aspectRatio: '1', display: 'grid', placeItems: 'center', flexShrink: 0 }}>
          ...
          {whyOpen && currentGameState?.type === 'move' && currentGameState?.quality && <div className="chess-game-sheet" role="dialog" aria-modal="true" aria-label="Why this verdict">
            ...
            <section className="chess-game-sheet__content">
              ...
              <div style={{ padding: '1rem 1.1rem', color: '#e8f6ff', lineHeight: 1.6, fontSize: '0.95rem' }}>
                {(() => { const why = buildWhyLesson({ fen: currentGameState.fen || '', classification: currentGameState.quality, movePhrase: currentGameState.movePhrase, bestMovePhrase: currentGameState.bestMovePhrase, captured: currentGameState.captured || null, check: String(currentGameState.move || '').includes('+'), mate: String(currentGameState.move || '').includes('#'), evalDelta: currentGameState.evalDelta ?? null, ply: currentGameState.ply || 0, move: currentGameState.move || null, bestMove: currentGameState.engineTelemetry?.bestMoveSan || currentGameState.engineTelemetry?.bestMove || null, fenBefore: currentGameState.fenBefore || null, engineLine: currentGameState.engineLine || null, sacrificePiece: currentGameState.sacrificePiece || null }); return <>
