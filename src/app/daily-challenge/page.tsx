...
export default function DailyChallengePage() {
  ...
  useEffect(() => { const current = getProfile(); setProfile(current); if (current.dailyAttempts[date]) setResult({ correct: true, timeMs: current.dailyAttempts[date].timeMs, points: current.dailyAttempts[date].points, message: 'You have already completed today’s challenge.' }); }, [date]);
  const chooseMove = (move: string) => {
    if (!profile || result?.correct) return;
    const firstMoveAt = startedAt ?? performance.now();
    ...
    const timeMs = Math.round(performance.now() - firstMoveAt);
