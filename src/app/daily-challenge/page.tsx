...
const PUZZLES = [
  { fen: 'r1bqkbnr/pppp1ppp/2n5/4p3/2B1P3/2NP1N2/PPP2PPP/R1BQK2R w KQkq - 4 4', answer: 'O-O', choices: ['O-O', 'Qe2', 'a3'] },
  { fen: 'r1bqk2r/pppp1ppp/2n2n2/2b1p3/2B1P3/2N2N2/PPPP1PPP/R1BQ1RK1 w kq - 4 6', answer: 'd4', choices: ['d4', 'h3', 'Re1'] },
  { fen: 'rnbqkbnr/pppp1ppp/8/4p3/4P3/5N2/PPPP1PPP/RNBQKB1R w KQkq - 1 2', answer: 'Bc4', choices: ['Bc4', 'a3', 'Ng1'] },
];
...
export default function DailyChallengePage() {
  ...
  return <main className="daily-page"><header className="daily-header"><div><span>CHESTER CHALLENGE · {date} UTC</span><h1>One Puzzle. 24 Hours.</h1></div><Link href="/">Town</Link></header><section className="daily-grid"><div className="daily-puzzle"><h2>Chester&apos;s Daily Breakthrough</h2><p>One shared position. Find the strongest move as quickly and accurately as you can. A correct solution is your one completed attempt for today.</p><div className="daily-board">{boardRows(puzzle.fen).flatMap((row, rank) => row.map((piece, file) => <span key={`${rank}-${file}`} className={(rank + file) % 2 ? 'dark' : 'light'}>{PIECES[piece] || ''}</span>))}</div><div className="daily-choices">{puzzle.choices.map((move) => <button key={move} onClick={() => chooseMove(move)} disabled={result?.correct}>{move}</button>)}</div>{result && <p className={result.correct ? 'daily-success' : 'daily-error'}>{result.message}{result.timeMs ? ` ${formatTime(result.timeMs)} · +${result.points} points.` : ''}</p>}</div><aside className="daily-leaderboard"><h2>Top 10 Today</h2><div className="daily-row daily-row--head"><span>Rank</span><span>Player</span><span>Time</span><span>Accuracy</span></div>{leaderboard.map((entry, index) => <div className="daily-row" key={`${entry.name}-${index}`}><span>#{index + 1}</span><b>{entry.name}</b><span>{formatTime(entry.time)}</span><span>{entry.accuracy}%</span></div>)}<small>Leaderboard entries are stored in this browser until shared accounts are configured.</small></aside></section></main>;
}