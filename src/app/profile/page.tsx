...
export default function ProfilePage() {
  ...
  return <main className="profile-page">
    <header className="profile-header"><div><span>CHESS TOWN PROFILE</span><h1>{profile.username}</h1><p>{title}</p></div><Link href="/">← Back to Chesterville</Link></header>
    ...
    <section className="profile-games"><WinRate label="VS CHESTER" played={profile.games.chester.played} won={profile.games.chester.won} color="#22d3ee" /><WinRate label="PVP ARENA" played={profile.games.pvp.played} won={profile.games.pvp.won} color="#f43f7a" /></section>
    <section className="profile-actions"><Link href="/chester-challenge">Chester Challenge</Link><Link href="/boss-map">Boss Map</Link><Link href="/play-chester">Play Chester</Link></section>
  </main>;
}